...
func Solve(g *Grid) bool {
	...
	// Work on a copy so g is only updated on success.
	copy := *g
	if solveHelper(&copy) {
		*g = copy
		return true
...
// solveHelper is the internal recursive backtracker for Solve.
func solveHelper(g *Grid) bool {
	...
	// No empty cell — solution complete.
	return true
}
